...
public class AfterSaleServiceImpl implements AfterSaleServiceApi {
    ...
    /** 商品 Feign 客户端（跨域查商品，用于商家售后列表） */
    private final ProductFeignClient productFeignClient;

    public AfterSaleServiceImpl(AfterSaleServiceMapper afterSaleServiceMapper, ServiceRecordMapper serviceRecordMapper, ProductFeignClient productFeignClient) {
    ...
    public IPage<AfterSaleService> getAfterSalesBySellerIdWithPage(Long sellerId, int page, int size, Integer status) {
        try {
            ...
            // 通过 Feign 查询商家商品（跨域调 product-service）
            List<Product> products = productFeignClient.listProductsBySeller(sellerId);

            System.out.println("===== [Service] 商家 " + sellerId + " 有 " + (products == null ? 0 : products.size()) + " 个商品 =====");

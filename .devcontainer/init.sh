...
echo "=========================================="
echo ""
echo "启动方式（详见 AGENTS.md 的 Startup Matrix；devcontainer 仅覆盖单体路径）："
echo ""
echo "  【单体路径（本容器内的权威入口）】"
echo "  1) 启动后端（端口 8081，MySQL/Redis 已由 devcontainer 自动启动）："
echo "     cd backend && mvn spring-boot:run"
echo ""
echo "  2) 启动用户商城（端口 5176）："
echo "     cd web-mall && npm run dev"
echo "     ⚠️ web-mall 的 dev 代理指向网关 :8080，单体路径下需改指 :8081 或走微服务路径"
echo ""
echo "  3) 启动管理后台（代理指向 :8081）："
echo "     cd admin-web && npm run dev"
echo ""
echo "  4) 启动商家端（代理指向 :8081）："
echo "     cd seller-web && npm run dev"
echo ""
echo "  【微服务路径（主线，需宿主机 Docker，devcontainer 内不支持）】"
echo "     docker compose -f docker-compose-infra.yml up -d   # Nacos/RocketMQ/Seata"
echo "     docker compose -f docker-compose-apps.yml up -d    # 网关 8080 + 6 微服务"
echo ""
